# AB-HPU / C0 / engineer review

A compact reservoir and removable equipment deck reserve space for a bought motor-pump, valve manifold and filtration. This is packaging CAD, not a designed hydraulic circuit.
...
Rebuild from repository: `uv venv --python 3.11 .cad-venv`, `uv pip install --python .cad-venv/bin/python -r engineering/cad/requirements.txt`, `.cad-venv/bin/python scripts/cad/build_models.py AB-HPU`. Packaged scripts are source references; run them in the documented repository layout.
